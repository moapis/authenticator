...
type userR struct {
	...
	Audiences AudienceSlice `boil:"Audiences" json:"Audiences" toml:"Audiences" yaml:"Audiences"`
...
var (
	...
	userColumnsWithoutDefault = []string{"email", "name", "created_at", "updated_at"}
...
func (userL) LoadPassword(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`auth.passwords`),
		qm.WhereIn(`auth.passwords.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for passwords")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for passwords")
	}
...
// LoadAudiences allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadAudiences(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.Select("\"auth\".\"audiences\".id, \"auth\".\"audiences\".created_at, \"auth\".\"audiences\".updated_at, \"auth\".\"audiences\".name, \"auth\".\"audiences\".description, \"a\".\"user_id\""),
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load audiences")
	}

	var resultSlice []*Audience
	...
	for results.Next() {
		...
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice audiences")
		}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on audiences")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for audiences")
	}
	...
	if singular {
		object.R.Audiences = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &audienceR{}
			}
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.Audiences = append(local.R.Audiences, foreign)
				if foreign.R == nil {
					foreign.R = &audienceR{}
				}
...
func (userL) LoadGroups(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"auth\".\"groups\".id, \"auth\".\"groups\".created_at, \"auth\".\"groups\".updated_at, \"auth\".\"groups\".name, \"auth\".\"groups\".description, \"a\".\"user_id\""),
		qm.From("\"auth\".\"groups\""),
...
// AddAudiences adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Audiences.
...
func (o *User) AddAudiences(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Audience) error {
	...
	if o.R == nil {
		...
		o.R.Audiences = append(o.R.Audiences, related...)
	}
...
// SetAudiences removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Users's Audiences accordingly.
...
func (o *User) SetAudiences(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Audience) error {
	...
	removeAudiencesFromUsersSlice(o, related)
	if o.R != nil {
		o.R.Audiences = nil
	}
	return o.AddAudiences(ctx, exec, insert, related...)
}

// RemoveAudiences relationships from objects passed in.
// Removes related items from R.Audiences (uses pointer comparison, removal does not keep order)
...
func (o *User) RemoveAudiences(ctx context.Context, exec boil.ContextExecutor, related ...*Audience) error {
	...
	for _, rel := range related {
		for i, ri := range o.R.Audiences {
			if rel != ri {
				continue
			}

			ln := len(o.R.Audiences)
			if ln > 1 && i < ln-1 {
				o.R.Audiences[i] = o.R.Audiences[ln-1]
			}
			o.R.Audiences = o.R.Audiences[:ln-1]
			break
		}
	}
...
func removeAudiencesFromUsersSlice(o *User, related []*Audience) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
